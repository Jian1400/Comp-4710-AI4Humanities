...
function renderRelatedLetterBrowser(data) {
  ...
  if (!letters.length) {
    ...
    empty.textContent = "No related letters are listed for this person.";
  ...
  function extractLetterYear(letter) {
    let title = cleanLetterTitle(letter);
    let matches = title.match(/\b(17|18|19)\d{2}\b/g);
  ...
  function extractCorrespondent(letter) {
    ...
    // Most archive titles place location/date after the recipient.
    let recipient = recipientAndRest.split(",")[0].trim();

    if (letter.relationType === "To") {
      return sender;
    }

    if (letter.relationType === "From") {
      return recipient;
    }
  ...
  letters.forEach(function(letter) {
    ...
    letter.year = extractLetterYear(letter);
  });
  ...
  controls.className = "LetterFilterControls";
  ...
  yearGroup.className = "LetterYearGroup";

  let yearHeading = document.createElement("div");
  yearHeading.className = "LetterFilterLabel LetterYearHeading";
  yearHeading.textContent = "Year";

  let yearRow = document.createElement("div");
  yearRow.className = "LetterYearRow";

  let fromYearSelect = document.createElement("select");
  fromYearSelect.className = "LetterFilterSelect LetterYearSelect";

  let toYearSelect = document.createElement("select");
  toYearSelect.className = "LetterFilterSelect LetterYearSelect";

  yearRow.appendChild(fromYearSelect);
  yearRow.appendChild(toYearSelect);
  ...
  function rebuildCorrespondentOptions() {
    let current = correspondentSelect.value || "all";
    let values = {};
    ...
    // About records typically do not have a clean sender/recipient.
    correspondentSelect.disabled =
      activeType === "About" || correspondents.length === 0;

    if (
      current !== "all" &&
      correspondents.indexOf(current) !== -1
    ) {
      correspondentSelect.value = current;
    } else {
      correspondentSelect.value = "all";
    }
  }

  function rebuildYearOptions() {
    let currentFrom = fromYearSelect.value || "all";
    let currentTo = toYearSelect.value || "all";

    let years = activeLetters()
      .map(function(letter) { return letter.year; })
      .filter(function(year) { return Number.isFinite(year); });

    let minYear = years.length ? Math.min.apply(null, years) : null;
    let maxYear = years.length ? Math.max.apply(null, years) : null;

    fromYearSelect.innerHTML = "";
    toYearSelect.innerHTML = "";

    if (minYear === null || maxYear === null) {
      let noFrom = document.createElement("option");
      noFrom.value = "all";
      noFrom.textContent = "No dates";
      fromYearSelect.appendChild(noFrom);

      let noTo = document.createElement("option");
      noTo.value = "all";
      noTo.textContent = "No dates";
      toYearSelect.appendChild(noTo);
    ...
    let allFrom = document.createElement("option");
    allFrom.value = "all";
    allFrom.textContent = "From " + minYear;
    fromYearSelect.appendChild(allFrom);

    let allTo = document.createElement("option");
    allTo.value = "all";
    allTo.textContent = "To " + maxYear;
    toYearSelect.appendChild(allTo);
    ...
    let validYears = years.map(String);

    if (
      currentFrom !== "all" &&
      validYears.indexOf(currentFrom) !== -1
    ) {
      fromYearSelect.value = currentFrom;
    } else {
      fromYearSelect.value = "all";
    }

    if (
      currentTo !== "all" &&
      validYears.indexOf(currentTo) !== -1
    ) {
      toYearSelect.value = currentTo;
    } else {
      toYearSelect.value = "all";
    }
  }

  function rebuildFiltersForActiveTab() {
    rebuildCorrespondentOptions();
    rebuildYearOptions();
  }

  function selectedYearRange() {
    let tabYears = activeLetters()
      .map(function(letter) { return letter.year; })
      .filter(function(year) { return Number.isFinite(year); });

    let minYear = tabYears.length ? Math.min.apply(null, tabYears) : null;
    let maxYear = tabYears.length ? Math.max.apply(null, tabYears) : null;

    let from =
      fromYearSelect.value === "all"
        ? minYear
        : Number(fromYearSelect.value);

    let to =
      toYearSelect.value === "all"
        ? maxYear
        : Number(toYearSelect.value);
  ...
  function normalizeYearRange(changedSelect) {
    let range = selectedYearRange();
    ...
    if (
      ...
      if (changedSelect === fromYearSelect) {
        toYearSelect.value = String(range.from);
      } else {
        fromYearSelect.value = String(range.to);
      }
    }
  }
  ...
  function draw() {
    ...
    let yearRange = selectedYearRange();
    ...
    let filtered = activeLetters().filter(function(letter) {
      ...
      if (
        Number.isFinite(yearRange.from) &&
        Number.isFinite(yearRange.to)
      ) {
        if (!Number.isFinite(letter.year)) {
          let fullRangeSelected =
            fromYearSelect.value === "all" &&
            toYearSelect.value === "all";

          if (!fullRangeSelected) return false;
        } else if (
          letter.year < yearRange.from ||
          letter.year > yearRange.to
        ) {
          return false;
        }
      }
  ...
  fromYearSelect.addEventListener("change", function() {
    normalizeYearRange(fromYearSelect);
    draw();
  });

  toYearSelect.addEventListener("change", function() {
    normalizeYearRange(toYearSelect);
    draw();
  });
